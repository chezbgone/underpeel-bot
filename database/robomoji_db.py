...
def _make_key(user_id: int, *layers: str):
    return {"id": _make_id(user_id), "sk": _make_sk(*layers)}
...
def get_emoji_info(user_id: int) -> RobomojiInfo | None:
    raw_response = the_table().get_item(
        Key=_make_key(user_id, "info"), ProjectionExpression="emojis, last_reacted"
    )
...
def toggle_emoji(
    ...
    if operation == "removed":
        ...
        the_table().update_item(
            Key=_make_key(user_id, "info"), UpdateExpression=f"REMOVE emojis[{i}]"
        )
    else:
        the_table().update_item(
            Key=_make_key(user_id, "info"),
